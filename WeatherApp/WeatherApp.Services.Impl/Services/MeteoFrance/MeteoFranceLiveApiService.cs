...
{
    internal class MeteoFranceLiveApiService : IMeteoFranceLiveApiService
    {
        ...
        private readonly ILogger<MeteoFranceLiveApiService> logger;
        private string token = "";
        private const string TokenCacheKey = "MeteoFranceToken";

        public MeteoFranceLiveApiService(
            IMeteoFranceFileReader meteoFranceFileReader,
            IMemoryCache memoryCache,
            ILogger<MeteoFranceLiveApiService> logger)
        {
        ...
        public async Task<string> GetStationDataAsync(string stationId, string type, DateTime beginDate, DateTime endDate)
        {
            this.logger.LogInformation($"{nameof(GetStationDataAsync)}({stationId}, {beginDate}, {endDate})");

            await this.InitializeAsync();

            var commandId = await this.RunAndRetryAsync(() => this.GetCommandStationAsync(stationId, type, beginDate, endDate));

            var csv = await this.RunAndRetryAsync(() => this.LoadCsvFromCommandIdAsync(commandId));
        ...
        private async Task<T> RunAndRetryAsync<T>(Func<Task<T>> taskAction)
        {
            const int maxTry = 3;
